...
para: inbox
aliases:
  - "2026-02-07-Eric-Buess-on-X-Claude-Code-Agent-Teams-What-I-Learned-from-Testing-X"
  - "Eric-Buess-on-X-Claude-Code-Agent-Teams-What-I-Learned-from-Testing-X"
---
# Eric Buess on X: "Claude Code Agent Teams: What I Learned from Testing" / X
...
![](https://pbs.twimg.com/profile_images/1809337055572316160/I1j9qHKg_bigger.jpg)
...
](https://x.com/EricBuess/article/2019817656745128366/media/2019811376181444610)
...
](https://x.com/EricBuess/status/2019817656745128366/analytics)
...
You describe the task and team structure in natural language. Claude creates the team, spawns teammates, distributes work, and manages coordination. Each teammate loads the same project context (

[CLAUDE.md](https://claude.md/)

, MCP servers, skills) but doesn't inherit the lead's conversation history.
...
I enabled it by adding an env block to my settings.json with CLAUDE\_CODE\_EXPERIMENTAL\_AGENT\_TEAMS set to "1". That persists across sessions. You can also just export the variable in your shell before launching, but the settings.json approach means you don't have to remember it every time.
...
I got split panes working in iTerm2 by installing the it2 CLI (see

[github.com/mkusaka/it2](https://github.com/mkusaka/it2)

for installation instructions), enabling the Python API in iTerm2 under Settings then General then Magic, and launching Claude Code with the \--teammate-mode tmux flag. The command I used was: claude --teammate-mode tmux
...
\- The first 200 lines of a

[MEMORY.md](https://memory.md/)

file in the memory directory get injected into the agent's system prompt

\- If

[MEMORY.md](https://memory.md/)

exceeds 200 lines, the agent is instructed to curate and trim it

\- Read, Write, and Edit tools are automatically enabled so the agent can manage its memory files

\- The agent can create additional topic-specific files and link to them from
...
files (project, user, rules) are for instructions — things you want Claude to always know and follow. Coding standards, project architecture, preferred patterns, common commands. These are static. You write them, Claude reads them.

Memory frontmatter on subagents is for learning — things the agent discovers on its own and builds up over time. That's the key difference.

[CLAUDE.md](https://claude.md/)

is "here are the rules." Agent memory is "here's what I've figured out." Agent memory is dynamic, agent-managed, and compounds across conversations.
...
Managed policy — OS-specific system path. On macOS it's at /Library/Application Support/ClaudeCode/CLAUDE.md. On Linux it's at /etc/claude-code/CLAUDE.md. On Windows it's at C:\\Program Files\\ClaudeCode\\
...
file in the project root or under .claude/. Team-shared via git.
...
file at ~/.claude/. Personal preferences across all projects.

Project local — A

[CLAUDE.local.md](https://claude.local.md/)

file in the project root. Personal, this project only.

All memory files load automatically when Claude starts.

[CLAUDE.md](https://claude.md/)

files support an

[@path/to](https://x.com/@path/to)

/import syntax for importing other files recursively (up to 5 hops deep). The /memory command edits files, and /init bootstraps a new project
...
I updated Claude Code to the latest version, then added CLAUDE\_CODE\_EXPERIMENTAL\_AGENT\_TEAMS set to "1" inside an env block in my settings.json (at ~/.claude/settings.json).
...
[https://github.com/ericbuess/claude-code-docs.git](https://github.com/ericbuess/claude-code-docs.git)
...
[https://docs.anthropic.com/en/docs/claude-code/agent-teams](https://docs.anthropic.com/en/docs/claude-code/agent-teams)

— Agent teams (main reference for most of the article)

\-

[https://docs.anthropic.com/en/docs/claude-code/sub-agents](https://docs.anthropic.com/en/docs/claude-code/sub-agents)

— Subagents and memory frontmatter
...
[https://docs.anthropic.com/en/docs/claude-code/settings](https://docs.anthropic.com/en/docs/claude-code/settings)

— Settings.json, teammateMode, env config

\-

[https://docs.anthropic.com/en/docs/claude-code/terminal-setup](https://docs.anthropic.com/en/docs/claude-code/terminal-setup)

— Terminal support for split panes
...
[https://github.com/mkusaka/it2](https://github.com/mkusaka/it2)
...
[12:57 AM · Feb 7, 2026](https://x.com/EricBuess/status/2019817656745128366)
...
Views](https://x.com/EricBuess/status/2019817656745128366/analytics)
...
[View quotes](https://x.com/EricBuess/status/2019817656745128366/quotes)