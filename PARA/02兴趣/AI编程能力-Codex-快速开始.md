...
---
> [!summary]
> 把 Codex 当作“会读代码，会跑命令，会改文件，还能根据反馈迭代”的队友。你负责目标、约束、验证和最终审阅，AI 负责高效执行。
...
- 已经会跑命令，但希望把 bug 修复、重构、测试、文档做成可复用流程

你将学会：
- 选对入口（IDE / CLI / Cloud）并把任务讲清楚
- 用“可验证”的提示词驱动 Codex 交付高质量改动
- 用接近 Google 软件工程实践的方式组织需求、设计、测试、评审和文档
...
> [!tip]
> 提示词越长不等于越好。比起“长篇规章制度”，短而明确的 Done、约束、验证更稳定。
...
> [!tip]
> 当提示词里出现“顺便”“同时”“把所有相关地方都优化一下”，质量通常会下降。
...
常见反模式：
- 强行要求“每一步都长篇解释过程”
- 一个提示里同时要求“重构 + 新功能 + 性能优化 + 风格清理”

建议：
- 让模型把 token 花在实现和验证，而不是写过程作文
- 复杂任务拆为多轮，每轮只做一件可验证的事

### 3.5 工具输出太长时，先收敛再分析

问题：`rg`、`git diff`、大日志会挤爆上下文，导致结论变差。

实用做法：
- 先缩小范围（目录、文件、关键字）
- 再用 `head`/`tail`/`--max-count` 取关键片段
- 粘贴日志时优先“报错前后关键片段”，并标注省略了哪些部分

进阶（自建工具层时）：
- 可把工具输出预算控制在约 10k tokens
- 超限时优先“前半 + 后半”，中间标注截断，而不是只保留开头

### 3.6 并行读信息（Parallel Tool Calling）
...
- 需要贴日志时先做脱敏（把用户数据、订单号、邮箱等替换成占位符）
...
### 7.3 进度播报：默认让它自动，不要强控格式

建议：
- 不要强行要求“每一步都输出详细进度和长篇解释”
- 更稳的方式是写清验收标准，让它把重心放在完成与验证
...
- 一次要太多：把任务拆小，质量会明显上升
- 强行要求过程作文：会分散模型在实现上的注意力
- 不审 diff：任何自动化改动都可能引入隐藏回归
- 工具输出太长不收敛：上下文被噪音占满，结论会变差

## 10. 参考与延伸

- 原始素材：[[../03素材/资源-codex-docs]]
...
- 建议你为每个项目维护一份 `AGENTS.md`，把“怎么跑测试/怎么验证/不允许做什么”写死，让 Codex 更像可控的团队成员
